...
model_name = 'sentence-transformers/all-MiniLM-L6-v2' # change to the model we're using  
batch_size = 16
num_epochs = 30
max_seq_length = 128
...
###### Read Datasets ######
sts_dataset_path = 'golden_data.tsv.gz'
qqp_dataset_path = 'elmore_vs_digital_fortress'

# Look at what these datasets look like an make my own
...
# We will train the cross encoder with our golden data
cross_encoder_path = 'output/cross-encoder/'+model_name.replace("/", "-")+'-'+datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

# We then use our trained encoder to label an unlabeled dataset
